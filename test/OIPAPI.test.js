...
describe('OIPAPI', () => {
	describe('OIPAPI Functions', () => {
		...
		it('GET Artifact by TXID | getArtifact()', async () => {
			...
			let response = await index.getArtifact(txid)
			expect(response.success).toBeTruthy()
			expect(response.artifact.getInternalTypeAndSubtype() === 'research-tomogram')
		});
		it('GET multiple artifacts by TXID | getArtifacts()', async () => {
		...
		it('GET an OIP041 Artifact by TXID | get041Artifact()', async () => {
			...
			let response = await index.get041Artifact(txid)
			expect(response.success).toBeTruthy()
		...
		it('GET an Alexandria Media Artifact by TXID | getAlexandriaMediaArtifact()', async () => {
			...
			let response = await index.getAlexandriaMediaArtifact(txid)
			expect(response.success).toBeTruthy()
		...
		it('GET Multipart via TXID', async () => {
			...
			let response = await index.getMultipart(mpTXID)
			// console.log(response)
			expect(response.success).toBeTruthy()
		...
		it('GET Multiparts via Reference', async () => {
			let ref = '8c204c5f39'
			let response = await index.getMultiparts(ref)
			expect(response.success).toBeTruthy()
		...
		it('GET Multiparts via Reference w/ Limit', async () => {
			let ref = '8c204c5f39'
			let limit = 3
			let response = await index.getMultiparts(ref, limit)
			expect(response.success).toBeTruthy()
		...
		it(`GET historian data point by ID | getHistorianData`, async () => {
			let id = '83452d60230d3c2c69000c2a79da79fe60cdf63012f946ac46e6df3409fb1fa7'
			let response = await index.getHistorianData(id)
			expect(response.success).toBeTruthy()
		})
