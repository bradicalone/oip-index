...
var _FloDataTX = _interopRequireDefault(require("./OIPComponents/FloDataTX"));
...
const hydrateArray = artifacts => {
  ...
  for (let art of artifacts) {
    tmpArray.push(new _Artifact.default(art));
  }
...
const hydrateFloDataTX = floDataTXs => {
  ...
  for (let data of floDataTXs) {
    tmpArray.push(new _FloDataTX.default(data));
  }
...
class OIPAPI {
  ...
  constructor(settings) {
    if (settings && settings.OIPdURL) {
      this.setOIPdURL(settings.OIPdURL); //ToDo: switch back to defaultOIPdURL
  ...
  async searchArtifacts(query, limit) {
    ...
    if (res && res.data) {
      ...
      if (artifacts.length === 0) return {
        success: true,
        warning: `No artifacts found with query: ${query}`,
        artifacts
      };
      return {
        success: true,
        artifacts: hydrateArray(artifacts)
      };
    } else {
      return {
        success: false,
        error: 'No data returned from axios request',
        response: res.data
      };
    }
  }
  ...
  async getArtifact(txid) {
    ...
    if (res && res.data) {
      ...
      if (resultArray.length === 0) {
        ...
        return {
          success: true,
          warning: "Multiple artifacts found, possible collision",
          artifacts: resultArray
        };
      } else return {
        success: true,
        artifact: new _Artifact.default(resultArray[0])
      };
    } else {
      return {
        success: false,
  ...
  async getArtifacts(txids) {
    ...
    for (let txid of txids) {
      ...
      if (res.success) artifacts.push(res.artifact);else errors.push(res.artifact);
    }

    if (errors.length > 0) {
      return {
        success: true,
        warning: 'Not [all artifacts] found',
        error: errors,
        artifacts
      };
    } else {
  ...
  async getLatestArtifacts(limit = 100, nsfw = false) {
    ...
    if (res && res.data) {
      ...
      if (artifacts.length > 0 && artifacts.length < limit) return {
        success: true,
        artifacts: hydrateArray(artifacts),
        warning: 'Not all requested artifacts were found'
      };
  ...
  async get041Artifact(txid) {
    ...
    if (res && res.data) {
      ...
      if (resultArray.length === 0) {
        ...
        return {
          success: false,
          error: "Multiple artifacts found, possible collision",
          artifacts: resultArray
        };
      } else return {
        success: true,
  ...
  async searchFloData(query, limit) {
    ...
    if (res && res.data) {
      let txs = res.data.results; //ToDo: consider not initializing a class for tx data

      return {
        success: true,
        txs: hydrateFloDataTX(txs)
      };
    } else {
      return {
        success: false,
        error: 'Missing axios data response',
        response: res.data
      };
    }
  }
  ...
  async getFloData(txid) {
    ...
    if (res && res.data) {
      ...
      if (results.length === 0) {
        return {
          success: false,
          error: 'No floData found',
          response: res.data
        };
      } else if (results.length > 1) {
        return {
          success: false,
          error: 'Collision: multiple data points found',
          response: res.data
        };
      } else {
        return {
          success: true,
          floData: res.data.results[0].tx.floData
        };
      }
    } else {
      return {
  ...
  async getMultipart(txid) {
    ...
    if (res && res.data) {
      ...
      for (let mp of results) {
        multiparts.push(new _MPSingle.default(mp));
      }
      ...
      if (!total) {
        ...
        return {
          success: false,
          message: "Collision: mulitple parts found with single ID",
          multiparts
        };
      } else {
        return {
          success: true,
          multipart: multiparts[0]
        };
      }
    } else {
      return {
