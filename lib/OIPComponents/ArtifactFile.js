...
class ArtifactFile {
  ...
  getLocation() {
    if (this.file.location) return this.file.location;
    if (this.parent) return this.parent.getLocation();
    return undefined;
  }
  ...
  getPaymentAddresses() {
    if (this.file.payment && this.file.payment.addresses && this.file.payment.addresses !== []) return this.file.payment.addresses;
  ...
  getPaymentScale() {
    ...
    if (this.file.scale) {
      ...
      if (this.parent) return this.parent.getPaymentScale();else return 1; // Default return one if we can't get the parents payment scale, and our payment scale is not set either
    }
  }
