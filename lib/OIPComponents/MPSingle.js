...
class MPSingle {
  ...
  isValid() {
    ...
    if (!this.getSignature()) {
      return {
        success: false,
        message: "Must have a Signature!"
      };
    } //signature validation


    if (!(0, _bitcoinjsMessage.verify)(this.getSignatureData(), this.getAddress(), this.getSignature(), '\u001bFlorincoin Signed Message:\n')) {
      return {
        success: false,
  ...
  toString() {
    if (this.isValid().success) {
      return this.prefix + "(" + this.getPart() + "," + this.getMax() + "," + this.getAddress() + "," + this.getReference() + "," + this.getSignature() + "):" + this.getData();
  ...
  signSelf(ECPair) {
    if (!ECPair) return {
      success: false,
      error: "No Private Key available! Unable to sign message!"
    };
    ...
    try {
      signature_buffer = (0, _bitcoinjsMessage.sign)(this.getSignatureData(), privateKeyBuffer, compressed, ECPair.network.messagePrefix);
    } catch (e) {
      return {
        success: false,
        error: e
