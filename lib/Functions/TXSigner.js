...
function sign(transactionBuilder, extraBytes, vin, keyPair, redeemScript, hashType, witnessValue, witnessScript) {
  ...
  let input = transactionBuilder.__inputs[vin]; // if redeemScript was previously provided, enforce consistency

  if (input.redeemScript !== undefined && redeemScript && !input.redeemScript.equals(redeemScript)) {
    throw new Error('Inconsistent redeemScript');
  }

  let ourPubKey = keyPair.publicKey; // Check both publickKey, getPublicKey, and getPublicKeyBuffer to support all HDNode types (bitcoinjs-lib v3 & bip32 npm & bitcoinjs-lib v4 when it comes out)
  ...
  let signed = input.pubkeys.some(function (pubKey, i) {
...
function hashForSignature(transaction, extraBytes, inIndex, prevOutScript, hashType) {
  ...
  if (inIndex >= transaction.ins.length) return ONE; // ignore OP_CODESEPARATOR

  let ourScript = _bitcoinjsLib.default.script.compile(_bitcoinjsLib.default.script.decompile(prevOutScript).filter(function (x) {
    return x !== 171; // OP_CODESEPARATOR
  }));

  let txTmp = transaction.clone(); // SIGHASH_NONE: ignore all outputs? (wildcard payee)
  ...
  if ((hashType & 0x1f) === _bitcoinjsLib.default.Transaction.SIGHASH_NONE) {
    ...
    if (inIndex >= transaction.outs.length) return ONE; // truncate outputs after
  ...
  let extraBytesString = extraBytes || ""; // Get the regular tx hex buffer

  let buffer = Buffer.allocUnsafe(txTmp.__byteLength(false));

  txTmp.__toBuffer(buffer, 0, false); // tx hex buffer to string (for appending data)


  let txHexStr = buffer.toString('hex'); // Append on Extra Bytes (floData)

  txHexStr += extraBytesString; // Create hashType buffer

  let hashTypeBuf = Buffer.allocUnsafe(4);
